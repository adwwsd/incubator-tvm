...
@autotvm.register_topi_compute("conv2d_hwnc_tensorcore_im2col.cuda")
def conv2d_hwnc_tensorcore_im2col(cfg, data, kernel, stride, padding, dilation, layout, out_dtype='int32'):
    """Convolution operator in HWNC layout for int4 using im2col method.
    Parameters
    ----------
    cfg: ConfigEntity
    ...
    in_height, in_width, batch_size, in_channels = get_const_tuple(data.shape)
    ...
    data_im2col = te.compute((im2col_M, im2col_K), lambda i, j: \
                                ...
                                pad_data[((i // out_width % out_height) * stride_h) + j // (in_channels * kernel_w),
                                (i % out_width * stride_w) + (j // in_channels) % kernel_w,
                                i // (out_height * out_width), j % in_channels],
    ...
    output = te.compute((out_height, out_width, batch_size, out_channels), lambda oh, ow, n, oc: \
                            ...
                            name='output', tag='conv2d_hwnc_tensorcore_im2col')
    return output

@autotvm.register_topi_schedule("conv2d_hwnc_tensorcore_im2col.cuda")
def schedule_conv2d_hwnc_tensorcore_im2col(cfg, outs):
    """Schedule conv2d HWNC im2col int4 template"""
    outs = [outs] if isinstance(outs, te.tensor.Tensor) else outs
    s = te.create_schedule([x.op for x in outs])

    def _callback(op):
        if op.tag == 'conv2d_hwnc_tensorcore_im2col':
            _schedule_conv2d_hwnc_tensorcore_im2col(cfg, s, op.output(0))

    traverse_inline(s, outs[0].op, _callback)
    return s

def _schedule_conv2d_hwnc_tensorcore_im2col(cfg, s, output):
    ...
    if cfg.is_fallback:
        ref_log = autotvm.tophub.load_reference_log(
            target.target_name, target.model, 'conv2d_hwnc_tensorcore_im2col.cuda')
